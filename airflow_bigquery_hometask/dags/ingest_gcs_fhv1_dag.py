'''
DAG for ingesting For-Hire Vehicle Trip Records to GCP.
Data interval: 2019 year.
'''
import os
# import logging
...
# Get interval start for the current DAG and get the previous month for downloading
EXECUTION_MONTH_TEMPLATE = "{{ data_interval_start.subtract(months=1).strftime(\'%Y-%m\') }}"

DATASET_FILE = f"fhv_tripdata_{EXECUTION_MONTH_TEMPLATE}.parquet"
...
with DAG(
    dag_id="ingest_gcs_fhv1_dag",
    schedule_interval="0 6 2 * *",
    start_date=datetime(2019, 2, 1), # The first DAG's start gets the data for Jan 2019
